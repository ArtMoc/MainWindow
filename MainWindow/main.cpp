...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInst, LPSTR lpCmdLine, INT nCmdShow)
{
	...
	//wc.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_ICON_TERM));
	//wc.hIconSm = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_ICON_WOOD));
	wc.hIcon = (HICON)LoadImage(hInstance, "storm.ico", IMAGE_ICON, 0, 0, LR_DEFAULTSIZE | LR_LOADFROMFILE);
	wc.hIconSm = (HICON)LoadImage(hInstance, "flower.ico", IMAGE_ICON, 0, 0, LR_DEFAULTSIZE | LR_LOADFROMFILE);
	...
	int windwod_width = screen_width - screen_width / 4;
	...
	(
		...
		windwod_width, window_height, //размер окна
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE: //создаются элементы окна
		break;
	case WM_COMMAND: //обработка команд нажатия кнопок и других элементов кона
		break;
	case WM_SIZE:
	case WM_MOVE:
	{
	...
	case WM_CLOSE:
		...
		{
		case IDYES:
			//MessageBox(hwnd, "Лучше бы двери закрыли :-)", "Info", MB_OK | MB_ICONERROR);
